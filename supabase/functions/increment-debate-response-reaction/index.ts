...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!responseId || !["naruhodo", "hmm"].includes(normalizedReaction)) {
      return jsonResponse({ error: "responseId and a valid reaction are required" }, 400);
    }
    ...
    const { data: responses, error: readError } = await supabase
      ...
      .in("id", actorResponseId ? [actorResponseId, responseId] : [responseId]);
    if (readError || !responses) {
      return jsonResponse({ error: "Failed to read responses", details: readError?.message }, 400);
    }

    const actor = responses.find((item) => item.id === actorResponseId);
    const target = responses.find((item) => item.id === responseId);
    if (!target || (actor && (actor.question_id !== target.question_id || actor.id === target.id))) {
    ...
    return jsonResponse({
      ...
      event: {
        actorSide: actor?.choice_side ?? null,
        targetSide: target.choice_side,
        relation: actor ? (actor.choice_side === target.choice_side ? "same" : "opposite") : "unanswered",
        reaction: normalizedReaction,
      },
    });
  } catch (error) {
